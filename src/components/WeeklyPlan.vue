...
import { dayName, dayNumber, periodRange, toDateKey } from '../utils/planner'

const planner = usePlannerStore()
const clearDialogOpen = ref(false)
const todayKey = toDateKey(new Date())
const weekBoardStyle = computed(() => {
  const todayIndex = planner.currentWeek?.days.findIndex((day) => day.date === todayKey) ?? -1
  const columns = Array.from({ length: 7 }, (_, index) =>
    index === todayIndex ? 'minmax(250px, 1.45fr)' : 'minmax(170px, 1fr)',
  )
